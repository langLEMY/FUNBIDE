...
public class ListarPacientesUseCaseTests
{
    ...
    public ListarPacientesUseCaseTests()
    {
        _pacienteRepository
            .ObtenerPaginadoAsync(
                Arg.Any<int>(), Arg.Any<int>(), Arg.Any<string?>(), Arg.Any<Domain.Enums.EstadoPaciente?>(),
                Arg.Any<Domain.Enums.OrdenPaciente>(), Arg.Any<CancellationToken>())
    ...
    public async Task EjecutarAsync_BusquedaDeUnSoloCaracter_LaIgnoraYNoLaMandaAlRepositorio()
    {
        ...
        await _pacienteRepository.Received(1).ObtenerPaginadoAsync(
            1, 50, null, null, Arg.Any<Domain.Enums.OrdenPaciente>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task EjecutarAsync_BusquedaDeDosCaracteres_LaManda()
    {
        ...
        await _pacienteRepository.Received(1).ObtenerPaginadoAsync(
            1, 50, "an", null, Arg.Any<Domain.Enums.OrdenPaciente>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task EjecutarAsync_TamanoPaginaMayorAlMaximo_SeClampeaAlMaximo()
    {
        ...
        await _pacienteRepository.Received(1).ObtenerPaginadoAsync(
            1, 100, null, null, Arg.Any<Domain.Enums.OrdenPaciente>(), Arg.Any<CancellationToken>());
    }
}
